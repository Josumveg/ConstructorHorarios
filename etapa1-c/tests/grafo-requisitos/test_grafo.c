...
/* Agrega un curso vacío (sin requisitos) al cátalogo y retorna su índice */
...
/* Caso 2: requisito con código inexistente (como CI1230 -> CI0200/CI0202) se omite sin romper los indices de los demas requisitos del curso. */
static void test_requisito_inexistente_se_omite(void) {
    ...
    /* el índice de CI1107 no se ve afectado */
    assert(grafo.cantidad_adyacentes[ci1107] == 0);

    printf("OK: requisito con código inexistente se omite sin romper índices\n");
}

/* Caso 3: ciclo A -> B -> A queda representado tal cual en el grafo. */
